        ...
        <DashboardWrapper>
            <div className="max-w-6xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-3xl font-black tracking-tight text-zinc-900 dark:text-white">My Organized Events</h1>
                        <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 mt-1">Manage and track registrations for meetups you have created.</p>
                    </div>

                    <Link href="/dashboard/events/new" className="inline-flex items-center justify-center gap-2 px-5 py-3 bg-violet-600 hover:bg-violet-750 text-white rounded-2xl text-sm font-bold shadow-lg shadow-violet-500/10 hover:shadow-violet-500/20 hover:-translate-y-0.5 active:translate-y-0 transition-all cursor-pointer w-full sm:w-auto text-center">
                ...
                {error && (
                    <div className="mb-6 p-4 bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30 rounded-2xl flex items-start gap-3">
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-red-800 dark:text-red-300">Failed to load events</h3>
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20">
                        ...
                        <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400">Retrieving your event portfolio...</p>
                    </div>
                ) : events.length === 0 ? (
                    <div className="border border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl p-8 lg:p-12 text-center flex flex-col items-center justify-center max-w-md mx-auto mt-8">
                        ...
                        <h2 className="text-lg font-bold text-zinc-900 dark:text-white">No Events Created Yet</h2>
                        <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 mt-2">You haven't published any meetups yet. Create your first event to start gathering community attendees!</p>
                        <Link href="/dashboard/events/new" className="mt-6 inline-flex items-center gap-2 px-5 py-2.5 bg-zinc-900 dark:bg-zinc-100 hover:bg-zinc-800 dark:hover:bg-zinc-200 text-white dark:text-zinc-900 rounded-xl text-xs font-bold transition-all">
                            Get Started
                        </Link>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {events.map((event) => {
                            ...
                            return (
                                <div key={event.id} className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/60 rounded-3xl overflow-hidden shadow-xl shadow-zinc-100/10 dark:shadow-none hover:shadow-2xl hover:border-zinc-300 dark:hover:border-zinc-700 hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
                                    ...
                                    <div className="relative h-44 w-full bg-zinc-100 dark:bg-zinc-950 shrink-0">
                                        ...
                                        <div className="absolute top-4 left-4 flex gap-2">
                                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase ${event.is_published ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20" : "bg-zinc-500/10 text-zinc-500 border border-zinc-500/20"}`}>{event.is_published ? "Published" : "Draft"}</span>
                                        </div>
                                    </div>

                                    {/* Event Text Metadata details */}
                                    <div className="p-5 flex flex-col flex-grow">
                                        <h3 className="text-lg font-bold text-zinc-900 dark:text-white line-clamp-1 mb-2 hover:text-violet-600 transition-colors">{event.title}</h3>

                                        <p className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 line-clamp-2 mb-4">{event.description}</p>
